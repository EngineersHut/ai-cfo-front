...
export class ReportService {
  ...
  async findOne(id: string, userId: string, company: any) {
    ...
    const collection = this.connection.collection<any>(this.getCollectionName(companyId));
    ...
    if (rMonth && rYear) {
      ...
      const growthData = await this.growthModel.findOne({ companyId, month: rMonth, year: rYear }).exec();
      ...
      if (!comparison) {
        const currentMonthData = await this.dashboardModel.findOne({ companyId, month: rMonth, year: rYear }).exec();
        ...
        const prevMonthData = await this.dashboardModel.findOne({ companyId, month: prevMonth, year: prevYear }).exec();

        if (currentMonthData) {
          const calcChange = (curr: number | null, prev: number | null) => {
            if (!prev || prev === 0) return (curr && curr > 0) ? 100 : ((curr && curr < 0) ? -100 : 0);
            return (((curr || 0) - prev) / Math.abs(prev)) * 100;
          };

          comparison = {
            revenueChangePercent: round2(calcChange(currentMonthData.revenue, prevMonthData?.revenue || 0)),
            expenseChangePercent: round2(calcChange(currentMonthData.totalExpenses, prevMonthData?.totalExpenses || 0)),
            profitChangePercent: round2(calcChange(currentMonthData.netProfit, prevMonthData?.netProfit || 0)),
          };
        }
      }
    }
    ...
    return {
      ...
      rawDataTable: comparison ? (await this.dashboardModel.findOne({ companyId, month: rMonth, year: rYear }).exec())?.expenseBreakdown?.map(item => ({
        category: item.name,
        value: item.value,
  ...
  async getReportRevenueTrend(id: string, period: string, userId: string, company: any) {
    ...
    if (period === 'monthly') {
      ...
      const revenueTrend = dashboardData.map((d) => {
        const monthName = new Date(d.year || 0, (d.month || 1) - 1).toLocaleString("default", { month: "short" });
        return {
          label: monthName,
          revenue: d.revenue || 0,
    ...
    if (period === 'weekly') {
      ...
      if (transactions.length > 0) {
        const weeklyMap = new Map<string, { revenue: number, netProfit: number }>();
        ...
        return { revenueTrend: Array.from(weeklyMap.entries()).map(([label, data]) => ({ label, revenue: data.revenue, netProfit: data.netProfit })) };
      } else {
        // Fallback: Mock weekly data from the last 3 months of dashboard data
        const recentMonths = await this.dashboardModel.find({ companyId }).sort({ year: -1, month: -1 }).limit(3).exec();
        const fallbackTrend = [];
        for (const m of recentMonths.reverse()) {
          const monthName = new Date(m.year || 0, (m.month || 1) - 1).toLocaleString('default', { month: 'short' });
          for (let w = 1; w <= 4; w++) {
            fallbackTrend.push({
              label: `${monthName} W${w}`,
  ...
  async getReportExpenseBreakdown(id: string, period: string, userId: string, company: any) {
    ...
    const colors = ["#4f46e5", "#e11d48", "#f59e0b", "#84cc16", "#06b6d4", "#a855f7"];
