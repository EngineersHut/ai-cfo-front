...
function NavItem({
  ...
  const className = `group flex items-center cursor-pointer transition-all duration-300 ease-in-out mb-[8px] last:mb-0 rounded-[8px] border tracking-[-0.045em] ${
    isCollapsed ? "w-[44px] justify-center px-0" : "w-[180px] px-3 gap-3"
  } h-[40px] ${
    active
  ...
  const content = (
    <>
      <span
        className={`shrink-0 transition-all duration-300 ${active ? "text-white dark:text-blue-400" : "text-slate-400 dark:group-hover:text-slate-300"}`}
      >
        {icon}
      </span>
      <span
        className={`text-[14px] font-normal leading-[20px] whitespace-nowrap transition-all duration-300 overflow-hidden font-inter ${
...
export default function Sidebar({
  ...
  const firstLetter = userProfile.name
    ? userProfile.name.trim().charAt(0).toUpperCase()
    : "M";

  return (
    <aside
      className={`bg-white border-r border-slate-100 dark:bg-slate-900 dark:border-slate-700 flex flex-col h-full transition-all duration-500 ease-in-out overflow-hidden ${
        isCollapsed ? "w-[80px]" : "w-[220px]"
      }`}
    >
      {/* Branding & Toggle Section */}
      <div
        className={`flex flex-col transition-all duration-300 ${isCollapsed ? "items-center pt-8 pb-4" : "px-[20px] pt-[14px] pb-[14px]"}`}
      >
        <div
        ...
        >
          ...
          >
            <div
              className={`shrink-0 flex items-center justify-center rounded-lg font-bold text-xl transition-all duration-300 bg-[#0f172a] text-white dark:bg-white dark:text-[#0f172a] ${isCollapsed ? "w-11 h-11" : "w-10 h-10"}`}
            >
              N
            </div>
            <div
              className={`flex flex-col leading-none transition-all duration-300 overflow-hidden ${isCollapsed ? "w-0 opacity-0" : "w-auto opacity-100"}`}
            >
              <span className="text-[14px] font-bold text-[#0f172a] dark:text-white whitespace-nowrap">
                North Quest
              </span>
      ...
      <nav className="flex-1 px-[20px] py-2">
        <p
          className={`text-[12px] font-normal leading-4 text-slate-400 tracking-normal mb-[12px] transition-all duration-300 overflow-hidden font-inter dark:text-slate-500 ${
            isCollapsed ? "opacity-0 h-0" : "opacity-100 h-auto px-1"
          }`}
        >
          Main
        </p>
      ...
      <div
        className={`p-[20px] border-t border-slate-50 flex flex-col dark:border-slate-700 ${isCollapsed ? "items-center" : ""}`}
      >
        <NavItem
          icon={<SettingsIcon />}
          label="Settings"
        ...
        <div
          className={`bg-[#2563eb] border border-blue-400/50 flex items-center text-white shadow-lg shadow-blue-200 transition-all duration-300 overflow-hidden dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100 dark:shadow-none ${
            isCollapsed
              ? "w-[44px] h-[44px] justify-center p-0 rounded-xl"
              : "w-[180px] h-[40px] px-[12px] py-[4px] gap-[12px] rounded-[8px]"
          }`}
        >
