...
// Static mock data since API is not integrated for Forecast yet
const rawSummary = {
  revenue: 420000,
...
const reduxRevenueData = defaultRevenueData.map((d) => ({
  month: d.name,
  revenue: d.revenue,
  profit: d.netProfit,
}));
...
const auditCompliance = 98;
const equityHealth = 92;
...
const dashboardInsights = [
  {
    ...
    percentage: "",
    description:
      "At the current Burn Rate ($32.8K/Month), cash reserves will grow by 56% by Sep 2025.",
  },
  {
    title: "Operating Margin Alert",
    percentage: "",
    description:
      "Operating margin is projected to scale from 28.4% to 34.2% if customer acquisition costs decrease.",
  },
  {
    title: "Burn Rate Risk Mitigation",
    percentage: "",
    description:
      "Under the Conservative scenario, a 15% drop in enterprise renewal rate would decrease runway.",
  },
];
...
const costEfficiency = {
  fixedCost: { value: 120000, trend: "Stable" },
  variableCost: { value: 200000, trend: "-5.2%" },
  totalExpenses: { value: 320000, trend: "+3.5%" },
};
...
export default function ForecastPage() {
  ...
  const [expenseTimeframe, setExpenseTimeframe] = React.useState<
    "Weekly" | "Monthly"
  >("Monthly");
  ...
  const activeHeader = FORECAST_HEADER_CONFIGS[companyType as IndustryEnum] ??
    FORECAST_HEADER_CONFIGS[IndustryEnum.TRANSPORTATION_AND_LOGISTICS] ?? {
      title: "Forecast",
      subtitle: "",
    };
  const currentKPIs =
    FORECAST_KPI_CONFIGS[companyType as IndustryEnum] ??
    FORECAST_KPI_CONFIGS[IndustryEnum.TRANSPORTATION_AND_LOGISTICS] ??
    [];
  ...
  useEffect(() => {
    ...
    const updated = months.map((monthName, index) => {
      ...
      const simRevenue = Math.round(
        baseRevenue * monthlyFactor * growthMultiplier,
      );
      const simExpenses = Math.round(
        baseExpenses * (1 + index * 0.02) * expenseMultiplier,
      );

      if (index === 0) {
        cashAccumulator = startingCash;
      } else {
        cashAccumulator = Math.max(
          0,
          cashAccumulator + (simRevenue - simExpenses),
        );
      }
  ...
  const dynamicExpenseData = React.useMemo(() => {
    ...
    return [
      {
        name: "Fixed Costs",
        value: Math.round((fixed / total) * 100),
        color: "#6366f1",
      },
      {
        name: "Variable Costs",
        value: Math.round((variable / total) * 100),
        color: "#f59e0b",
      },
    ];
  }, [costEfficiency]);
  ...
  const dynamicCostDetails = React.useMemo(() => {
    ...
    return [
      {
        name: "Fixed Costs",
        value: costEfficiency.fixedCost?.value
          ? formatCurrency(costEfficiency.fixedCost.value)
          : "N/A",
      ...
      {
        name: "Variable Costs",
        value: costEfficiency.variableCost?.value
          ? formatCurrency(costEfficiency.variableCost.value)
          : "N/A",
      ...
      {
        name: "Total Expenses",
        value: costEfficiency.totalExpenses?.value
          ? formatCurrency(costEfficiency.totalExpenses.value)
          : "N/A",
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-[16px] pt-[4px] pb-[4px]">
        <div className="space-y-1">
          <h1 className="text-[24px] font-semibold text-slate-800 dark:text-slate-100 font-inter leading-[32px] tracking-tight">
            {activeHeader.title}
          </h1>
        ...
        <div className="flex items-center p-[4px] bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/80 rounded-[12px] shadow-sm shrink-0 gap-1">
          {(["Monthly", "Quarterly", "Yearly"] as const).map((option) => (
            <button
              key={option}
              onClick={() => setTimeframe(option)}
              className={`py-[8px] px-[16px] text-[13px] font-medium rounded-[8px] transition-all duration-300 ${
                timeframe === option
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {currentKPIs.map((metric, i) => {
          ...
          if (metric.key === "opMargin") {
            valueStr =
              rawSummary?.revenue > 0 ? `${margin.toFixed(1)}%` : "N/A";
          } else if (metric.key === "burnRate") {
            valueStr = rawSummary?.totalExpenses
              ? formatCurrency(rawSummary.totalExpenses)
              : "N/A";
          } else if (metric.key === "runway") {
            valueStr = rawSummary?.cashRunway
              ? `${rawSummary.cashRunway} months`
              : "N/A";
          } else if (metric.key === "cashInBank") {
            valueStr = rawSummary?.cashBalance
              ? formatCurrency(rawSummary.cashBalance)
              : "N/A";
          }
          ...
          return (
            <KPICard
              ...
              progress={
                metric.key === "opMargin" && rawSummary?.revenue > 0
                  ? margin
                  : undefined
              }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 w-full h-[490px] bg-white dark:bg-slate-800 rounded-[12px] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
          {/* Chart Header - 68px */}
          <div className="h-[68px] flex items-center justify-between p-[12px] gap-[12px] border-b border-slate-50 dark:border-slate-700/50">
          ...
          <div className="h-[412px] flex-1 w-full relative py-[12px] px-[16px] flex flex-col gap-[12px]">
            <div className="flex-1 w-full relative rounded-[10px] border border-[rgba(26,21,83,0.08)] dark:border-slate-700/50 bg-slate-50/30 dark:bg-slate-800/50 dark:bg-slate-800/50 flex flex-col overflow-hidden">
              ...
              {reduxRevenueData && reduxRevenueData.length > 0 && (
                <div className="flex items-center justify-center gap-8 py-3 bg-white dark:bg-slate-800 border-t border-[rgba(26,21,83,0.08)]">
        ...
        <div className="space-y-4">
          ...
          <div className="h-[330px] bg-white dark:bg-slate-800 rounded-[12px] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
            {/* Header - 54px */}
            <div className="h-[54px] flex items-center p-[12px] gap-[12px] border-b border-slate-50 dark:border-slate-700/50">
              <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-300">
                <Pin size={16} />
              </div>
              <h3 className="text-[16px] font-normal text-slate-800 dark:text-slate-100 font-inter leading-[24px] tracking-[0%]">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="px-5 space-y-3 border-t border-slate-50 dark:border-slate-700/50 pt-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[12px] font-semibold text-slate-900 dark:text-slate-100 font-inter leading-none">
                    {auditCompliance ? `${auditCompliance}%` : "N/A"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[12px] font-normal text-slate-700 dark:text-slate-300 font-inter leading-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-800 rounded-[12px] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
          <div className="h-[54px] flex items-center justify-between p-[12px] border-b border-slate-50 dark:border-slate-700/50">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-300">
                <PieChart size={16} />
              </div>
            ...
            <div className="flex gap-1">
              <button
                ...
                className={`w-[72px] h-[30px] px-[12px] py-[5px] flex items-center justify-center gap-[6px] rounded-[8px] text-[12px] font-medium transition-all duration-200 ${
                  expenseTimeframe === "Weekly"
                    ...
                    : "bg-white dark:bg-slate-800 border border-[#e2e8f0] dark:border-slate-600 text-slate-500 dark:text-slate-400 shadow-[0px_2px_4px_0px_rgba(0,0,0,0.08)] hover:text-slate-700"
                }`}
              >
                Weekly
              </button>
            ...
            {dynamicExpenseData.length > 0 ? (
              <div className="relative w-full h-full flex items-center justify-center min-h-0 min-w-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mt-[-10px]">
                  ...
                  <span className="text-[20px] font-bold text-slate-800 dark:text-slate-100 font-inter leading-none">
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-3  w-full ">
                {dynamicExpenseData.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-[13px] font-medium text-[#0a092e] dark:text-slate-200 font-inter leading-[20px] tracking-[0%]">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-800 rounded-[12px] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
          <div className="h-[54px] flex items-center p-[12px] border-b border-slate-50 dark:border-slate-700/50">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-300">
          ...
          <div className="p-6 flex flex-col justify-center flex-grow space-y-6">
            {dynamicCostDetails.length > 0 ? (
              dynamicCostDetails.map((item: any, idx: number) => (
                <div
                  key={idx}
                  className="space-y-2 pb-4 last:pb-0 border-b last:border-0 border-slate-50 dark:border-slate-700/50"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div
                      ...
                      />
                      <span className="text-[14px] font-normal text-[#0a092e] dark:text-slate-200 font-inter">
                        {item.name}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-[14px] font-bold text-slate-800 dark:text-slate-100 font-inter">
                        {item.value}
                      </span>
                      {item.trend && item.trend !== "Stable" && (
                        <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-[4px] bg-[#f0fdf4] dark:bg-emerald-900/30 border border-[#dcfce7] dark:border-emerald-800 text-[#22c55e] dark:text-emerald-400 text-[11px] font-semibold font-inter">
                  ...
                  <div className="w-full h-[10px] bg-[#eff6ff] dark:bg-slate-700 rounded-full overflow-hidden">
                    <div
              ...
              {activeInsights.map((item: any) => (
                <div key={item.id} className="flex gap-2 group">
                  ...
                  <div className="space-y-1">
                    ...
                    {/* Insight Text */}
                    <p className="text-[14px] text-slate-600 dark:text-slate-300 font-inter font-normal leading-[20px] tracking-[0%] align-middle">
                      {item.description}
                    </p>
                  </div>
                </div>
