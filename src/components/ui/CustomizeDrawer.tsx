...
export default function CustomizeDrawer({ isOpen, onClose }: CustomizeDrawerProps) {
  return (
    <>
      ...
      <div 
        className={`fixed top-0 right-0 h-full w-[340px] bg-white shadow-[-10px_0_40px_rgba(0,0,0,0.08)] z-[101] transform transition-transform duration-500 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
                  ...
                  <div key={section.id} className="w-[292px] border border-slate-100 rounded-[10px] bg-white flex flex-col overflow-hidden mx-auto shadow-sm">
                    <div className="px-4 py-3 border-b border-slate-50 flex items-center justify-between bg-white">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button className="text-[12px] text-blue-600 font-medium hover:underline">Hide all</button>
                      </div>
                    </div>
        ...
        {/* Footer Actions - Height 56px with Precise Padding */}
        <div className="h-[56px] border-t border-slate-100 bg-white flex items-center justify-between pt-[12px] pr-[18.17px] pb-[12px] pl-[17.95px] shrink-0">
          ...
          <button className="flex items-center justify-center gap-2 w-[135px] h-[31px] rounded-[7px] border border-slate-100 bg-white text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition-all shadow-sm">
...
function KPIToggleItem({ icon, label, sub }: { icon: string, label: string, sub: string }) {
    ...
    <div className="px-4 py-3 flex items-center justify-between group hover:bg-slate-50/50 transition-all">
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          <span className="text-[14px] font-medium text-slate-800 leading-tight">{label}</span>
          <span className="text-[11px] text-slate-400 leading-tight">{sub}</span>
        </div>
      </div>
      <button 
        onClick={() => setEnabled(!enabled)}
        className={`w-[36px] h-[20px] rounded-full relative transition-all duration-300 ${enabled ? 'bg-[#2563eb]' : 'bg-slate-200'}`}
      >
        <div className={`absolute top-0.5 left-0.5 w-[16px] h-[16px] bg-white rounded-full transition-all duration-300 shadow-sm ${enabled ? 'translate-x-[16px]' : 'translate-x-0'}`} />
      </button>
    </div>
  );
}
